...
class SubscriberMsgApi extends Model
{
 ...
 //    }
    ...
    public function updateData($mobile,$user_id,$security_code){
     //$this->db=$this->getDi()->getShared('db');
     $phql = "UPDATE `subscriber` s,`user`u SET s.ustatus = 'Verified' where s.mobile=$mobile AND u.mobile=$mobile AND u.id=$user_id AND security_code='$security_code' ";
 ...
 public function updateDataOTP($subscriber_id,$security_code)
    {
        $phql = "UPDATE `subscriber` s SET s.ustatus = 'Verified' where  s.id=$subscriber_id AND s.security_code='$security_code' ";
        $res  = $this->db->query($phql);
        return $res;
    }
    public function verifySubscriber($mobile,$access_date,$mg_id){ 
        //$phql = "SELECT count(*) as count, s.id, s.reg_id, s.app_type,s.jid,s.mobile FROM subscriber s, permissions p WHERE s.mobile IN (".implode(",",$mobile).") AND s.ustatus =  'Verified' AND s.last_date_access >=  '".$access_date."' AND s.id = p.subscriber_id AND p.mg_id=".$mg_id." AND p.status =  'Yes' group by s.mobile";
    $phql = "SELECT count(*) as count, s.id, s.reg_id, s.app_type,s.jid,s.mobile,s.notification_status FROM subscriber s WHERE s.mobile IN (".implode(",",$mobile).")  AND s.ustatus =  'Verified'  group by s.mobile";
    //    $phql = "SELECT count(*) as count, s.id, s.reg_id, s.app_type,s.jid,s.mobile,s.notification_status FROM subscriber s WHERE s.mobile IN (".implode(",",$mobile).") AND s.app_type !='Web' AND s.ustatus =  'Verified' AND s.last_date_access >=  '".$access_date."' group by s.mobile";
   ...
   public function getSubsciberByPincode($pincode){
        ...
        $phql = "SELECT mobile FROM subscriber WHERE (app_type!='Web' AND ustatus='Verified') AND (pincode_home IN(".$pincode.") OR pincode_work IN(".$pincode."))";
