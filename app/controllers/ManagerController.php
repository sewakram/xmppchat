...
/*    private function get_client_ip() {
    ...
    public function setslabAction($id)
    {
         ...
         if(!$id) {
          ...
          $request=$this->request;
          $permissions=$this->db->query("SELECT sp.value,p.id,p.feature FROM permissions p ,manager_permissions sp WHERE p.id=sp.permission_id AND sp.manager_id=$id");
         $permissions->setFetchMode(Phalcon\Db::FETCH_ASSOC);
         $permissions=$permissions->fetchAll();
        if ($this->request->isPost()) {
            foreach ($permissions as $key => $value) {
                    $perm = ManagerPermissions::findFirst("manager_id=".$id." AND permission_id=".$value['id']."");
         ...
         $this->view->setVars(array("permissions"=>$permissions));
    }
    public function indexAction()
    {
        ...
        //     $bal1=$user_bal->balance;
            ...
            // $bal1=$user_bal->balance ;
            $users = User::find("company_id= ".$user_id);
        }
         ...
         if ($this->request->isPost()){  
            ...
            if(!empty($senderid)){
                ...
                $senderid->save();
                 ...
                 $senderid = new SenderID();
                 $senderid->text=$request->getPost('senderid');
            // echo"<pre>";print_r($senderid->text);die();
                 $senderid->user_id=$id;
                 $senderid->status="Approved";
                 $senderid->save();
            }
            ...
            if ($user->save() == false && $senderid->save()==false) {
                foreach ($user->getMessages() as $message) {
                    $this->flash->error((string) $message);
                }
            } else {
...
// print_r($auth);exit;
        ...
        if ($this->request->isPost()) {
            ...
            // $user->slab_id=$slab->id;
            $user->sms_quota = 0;
            $company_exp = User::findFirst($auth['id']);
            $company_exp->expiry_date;
            $user->expiry_date =$company_exp->expiry_date;
            // $this->utils->getExpiryDate("NOW",$slab->validity,"add");
            //$user->slab_id=$request->getPost('slab_id','int');
            $msg_result=$this->utils->curl_redirect('http://sancharapp.com/msg-api/web_register','pincode='.$user->pincode.'&name='.$user->firstname.'&type=LinkApp&mobile='.$user->mobile);
            ...
            if ($user->save() == false) {
                ...
                return $this->response->redirect("manager/new");
            } else {
                copy('profile/default.jpg','profile/'.$user->mg_id.'.jpg');
                ...
                /*-------------------------------------------*/
                $this->welcomeMailSMS($user,$password);
                $this->flash->success('Manager Created');
                $cpermissions=$this->db->query("SELECT sp.id,sp.value,sp.permission_id FROM user u ,slab_permissions sp WHERE u.slab_id=sp.slab_id AND u.id='".$auth['id']."'");
                $cpermissions->setFetchMode(Phalcon\Db::FETCH_ASSOC);
                $cpermissions=$cpermissions->fetchAll();
                foreach ($cpermissions as $key => $cpermission) {
                $ManagerPermissions = new ManagerPermissions();
                $ManagerPermissions->manager_id=$user->id;
                $ManagerPermissions->permission_id=$cpermission['permission_id'];
                $ManagerPermissions->value=$cpermission['value'];
                if ($ManagerPermissions->save() == false) {
                foreach ($ManagerPermissions->getMessages() as $message) {
                $this->flash->error((string) $message);
                }
                }
                }
        ...
        //  }
         ...
         if ($request->isPost()){
            ...
            if ($config->save() == false) {
                ...
                return $this->response->redirect("manager/configuration");  
            }
