...
<div class="compose_wrapper">
  <form name="composeForm" id="composeForm" method="post" autocomplete="off" action="" class="form-horizontal" role="form">
      <div class="row" id="myoptions">
        <?php if($user_permission['contact_groups'] == 'Yes'){?>
        <div class="col s4">
        <input class="with-gap" onclick="handleClick('group_id');" id="group_type_msg" type="radio" value="group" name="to_type_msg"  <?php if($user_permission['contact_groups'] == 'Yes'){echo "checked";}?> /><label for="group_type_msg">Group</label></div>
        <?php } ?>
        <?php if($user_permission['pincode_message'] == 'Yes'){?>
        <div class="col s4">
        <input class="with-gap" onclick="handleClick('pincode');" id="public_type_msg" type="radio" value="pincode" name="to_type_msg"/> <label for="public_type_msg">Public Service</label></div>
        <?php } ?>
        <div class="col s4">
        <input class="with-gap" onclick="handleClick('single');" id="single_type_msg" type="radio" value="single" name="to_type_msg" <?php if($user_permission['contact_groups'] == 'No'){echo "checked";}?> /> <label for="single_type_msg">Numbers</label></div>
        <?php if($user_permission['excel_message_upload'] == 'Yes'){?>
        <div class="col s4">
        <input class="with-gap" onclick="handleClick('excelfile');" id="excelfile_type_msg" type="radio" value="excelfile" name="to_type_msg"/><label for="excelfile_type_msg">File Upload</label></div>
        <?php } ?>
      </div>
            <div id="to_div">
        <?php 
            if($user_permission['pincode_message'] == 'Yes'){?>
                    <div id="pincode" class="row hide select_Wrapper">
                      <?php
                    $pincode=array();
                    ...
                    <?php }
                     if($user_permission['excel_message_upload'] == 'Yes'){
                       echo '<div id="excelfile" class="row hide select_Wrapper"><div class="file-field input-field">
      <div class="btn"><span>File</span><input type="file" name="excelfile" id="excelfile" /></div><div class="file-path-wrapper"><input class="file-path validate" type="text"></div></div><p class="grey-text">Allowed File Types are .TXT or .XLS or .XLSX <a href="/sample.xls" download>Download sample file</a></p></div>';
                    ...
                    $class_temp='';
                    if($user_permission['contact_groups'] == 'Yes'){$class_temp='hide';}
                    ...
                    ?>
                      <div id="group_id" class="row <?php if($user_permission['contact_groups'] == 'No'){echo 'hide';}?> select_Wrapper">
