                      ...
                      <!--<input type="file" name="csvimport" id="csvimport" class="form-control" />-->
                        <div id="excelfile" class="row select_Wrapper">
                          <div class="file-field input-field">
                           ...
                           <p class="help-block">Allowed File Types are .XLSX or .XLS <a href="/dynamic_msg_upload.xls" download>Download sample file</a></p>
                        </div>
                    </div>
...
?>
<!--start-->
 <div class="col s6" style="padding-left:0;display:none;">
   ...
   <p class="help-block">Column (A) is for Mobile Numbers.</p>
<!--end-->
...
<div class="row message-tab">
    ...
    <div id="text-option1" class="col s12">
      ...
      <label for="option_uni">Unicode</label> 
        ...
        <textarea name="message_val"   onKeyUp="countChars(this)" onFocus="countChars(this)"   id="message_val" class="textarea text-message-row materialize-textarea" placeholder="Type a message"></textarea><div id="charNumber" align="left" style="color:#000000;"></div>
      <div id="message_error" align="left" style="color:#F00;display:none"><strong>Please enter message</strong></div>
...
<button id="submitbtn" type="submit" class="smart_btn circle right send_text compose_common waves-effect waves-light relative"><i class="center material-icons">send</i></button>
 ...
 <!--modal start-->
                ...
                <div id="addgroupmod" class="modal">
                 ...
                 <div id="summarymodals" class="modal">
                <h3 class="boot_dialog">Message Summary</h3><hr>
                 <h4><b id='total_msg_sents'></b> Message sent<br><b id='linkapp_user_counts'></b> Sancharapp user</h4>
                <div class='modal-footer'>
                      <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Cancel</a>
...
var char_limit = "<?php echo $char_limit ?>";
  ...
  function countChars(val) {
      uni =0;
      if ($('input#option_uni:checked').length > 0) 
        uni =1;
      if(uni==0){
        ...
        if (len >= max) {
          ...
          $('#charNumber').html('<b style=\'color:red;\'> you have reached the limit</b>');
        } else {
          if(len>160)
            var smscount=Math.ceil(len/153);
          else
            var smscount=Math.ceil(len/160);
          $('#charNumber').html("");
          document.getElementById("message_error").style.display='none';
          $('#charNumber').append("Enterd Characters:<b style=\'color:red;\'>"+len+"</b>  Total SMS(s):<b style=\'color:red;\'>"+smscount+"</b> Max Length:<b style=\'color:red;\'>"+max+"</b>");
        }
      }
      else{
        var len = val.value.length;
        ...
        if (len >= max) {
          ...
          if(len>70)
            var smscount=Math.ceil(len/64);
          else
            var smscount=Math.ceil(len/70);
          $('#charNumber').html("");
          $('#charNumber').append("Enterd Characters:<b style=\'color:red;\'>"+len+"</b>  Total SMS(s):<b style=\'color:red;\'>"+smscount+"</b> Max Length:<b style=\'color:red;\'>"+max+"</b>");
        }
    ...
    /////////////////strat
                           ...
                           //      $("#successmsg").html('<div class="alert alert-success">Message sent</div>').show();
                               $("html, body").animate({ scrollTop: 0 }, "slow");
                                ...
                                var not_in_group_array=response['not_in_group_array'];
                                var not_in_group_array_html='';
                                document.getElementById("total_msg_sents").innerHTML = total_msg_sent;
                                
                               if(linkapp_user_count==""){
                                linkapp_user_count=0;
                               }
                                document.getElementById("linkapp_user_counts").innerHTML = linkapp_user_count;
                               $('#summarymodals').openModal();
                               }
                               return false;
                            }
                      });
